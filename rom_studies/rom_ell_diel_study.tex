...
as an analyte to simulate the behavior of a bioconjugate sensor. However, they use a extremely simplified models
...
study the consequences of these simplifications. We analyzed the effects of the model chosen 
...
in computational models. This chapter covers the results of using different assumptions in the modelling process and what 
...
model consists of finding the dimensions of an ellipsoid that encapsules all the charges within the protein. We determined the principal axes of this
...
Since the PCA model can encapsule all the charges we use this model to study the effect of using a distribution of charges, versus
...
the different representations of cahrges (full distribution and equivalent monopole) as well as the impact of having
no charges. The setup of the simulations consists in a silver sphere sensor ($r=8$ nm) with a PCA ellipsoid 
...
allow us to explore the effects of the volume equivalent model. Similarly to the PCA case, we present a convergence study for the volume  n section \ref{sssec:ve_conv}. 
...
over estimates the original by $50$ $\%$ (shift $0.75$ nm), but we can say that a reduced order model based on the volume 
is a closer representation of the full protein. In both cases we can see that the intensity of the peak decreases. This can be 
a problem, along with the overestimation of the shift,
...
conclusions of sensitive capabilities of a sensors, based on models that under estimate the magnitude of the peak and 
...
and the difference between the intensities at the peak is $<1\%$ (computations results in reproducibility materials, section \ref{sec:repro_ell}).
...
Figure \ref{fig:two_ell_mult_config} shows the LSPR response of a silver nanoparticle and how it changes when the ellipsoids 
are placed in different orientations, for the different configurations. The peak when the dipole moment is parallel to the $y$-axis (original)
...
all these effects are not capture due to the nature of the geometry, which can lead to misleading conclusions. Even though, 
we know that using a volume equivalent ellipsoid model over estimates the shift, it stills captures the effect of the orientation, making
...
that volume equivalent ellipsoids over estimate the response, they still account for the effects of 
...
These results tells us that the LSPR response is not only affected by the orientations but also by the number of protein 
...
for the protein, by keeping the water dielectric as complex, secondly we analyze the effect of the medium by using a real-valued 
...
Once we understood how the dielectric model affected the LSPR response of the isolated nanoparticle, we study how this 
...
the generalization of the water dielectric (refractive index $n=1.33$), the peak for the extinction cross section when the two BSA proteins are present occurs at a wavelength of 
...
the complex dielectric for the water or an average of the real part of it. However, when using a different real value for the dielectric, like the generalization $e=1.7689$, it
not only affects the wavelength at which the resonance peaks occur but also the shift when proteins are added in the vicinity. 
...
we used the average valued of the real component from Phan et al. \cite{PhanETal2013}, and for the water medium we used the same two dielectrics cases
...
generalization case. These results tells us that, when it comes to the shift in the LSPR response, there is no difference between using a complex dielectric 
value for the protein and the medium, and using an averaged value of the real part. However, there is a difference in the magnitude of the 
